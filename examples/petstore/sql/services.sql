...
create service if not exists user_service (
    ...
    get_user(user_id varchar) varchar comment 'user和account信息用json返回，对应key就是user和account'
);

create service if not exists store_service comment '实现方法时要返回关联表的信息'(
    add_product(product Product) void,
    get_all_categories() varchar comment 'categories信息用json返回，对应key就是categories',
